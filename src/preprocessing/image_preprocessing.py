...
USE_EASYOCR = OCR_BACKEND == "easyocr"
...
if USE_EASYOCR:
	try:
		...
		print("ℹ EasyOCR available")
	except (ImportError, OSError, Exception) as e:
		# Catch DLL errors, import errors, etc.
		EASYOCR_AVAILABLE = False
		_easyocr_reader = None
		print(f"ℹ EasyOCR not available ({type(e).__name__}), using Tesseract")
else:
	EASYOCR_AVAILABLE = False
	_easyocr_reader = None
...
def _normalize_tesseract_lang(lang: str) -> str:
	"""Map common short language codes to Tesseract language identifiers."""
	if not lang:
		return 'deu'
	...
	parts = [part.strip().lower() for part in lang.split('+') if part.strip()]
	if not parts:
		return 'deu'
...
def get_easyocr_reader():
	...
	if _easyocr_reader is None and EASYOCR_AVAILABLE:
		print("Initializing EasyOCR (German + English)...")
...
def ocr_image(img: np.ndarray, lang: str = 'deu', config: str = '', method: str = 'auto') -> str:
	"""Unified OCR interface supporting both EasyOCR and Tesseract.
	
	Args:
		img: Grayscale or BGR image
		lang: Language ('deu'/'de' for German, 'eng'/'en' for English)
		config: Tesseract config string (ignored for EasyOCR)
		method: 'auto' (prefer EasyOCR), 'easyocr', or 'tesseract'
	
	Returns:
		Extracted text
	"""
	use_easyocr = (method == 'easyocr') or (method == 'auto' and USE_EASYOCR and EASYOCR_AVAILABLE)
	...
	tesseract_lang = _normalize_tesseract_lang(lang)
