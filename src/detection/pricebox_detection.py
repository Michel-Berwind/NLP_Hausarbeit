...
def detect_prices(
	...
	# Remove near-duplicate detections from the two detection passes
	if not detections:
		return detections
	...
	for det in detections:
		...
		for kept_det in kept:
			if compute_iou(tuple(box), tuple(kept_det["box"])) >= 0.6:
				is_duplicate = True
				break
		if not is_duplicate:
			kept.append(det)
...
def extract_price_from_crop(crop: np.ndarray) -> str:
	...
	# Preprocess: resize larger for better OCR (increased from 4x to 5x)
	resized = cv2.resize(crop, None, fx=5.0, fy=5.0, interpolation=cv2.INTER_CUBIC)
	gray = cv2.cvtColor(resized, cv2.COLOR_BGR2GRAY)
	
	# Enhanced preprocessing for better digit recognition
	# Removed denoising (too slow, causing hangs) - CLAHE alone is sufficient
	...
	def add_price_candidate(price: str, base_score: int) -> None:
		"""Add a price candidate and likely OCR-corrected variants."""
		try:
		...
		price_candidates.append((price, base_score))

		# Common OCR confusion: 99 is often read as 29/27/79/77
		if cents in {'27', '77', '79'}:
			price_candidates.append((f"{euros}.99", base_score + 6))
		elif cents == '29':
			# Keep 1.29 plausible, but strongly prefer .99 for higher prices
			bonus = 5 if int(euros) >= 2 else 1
			price_candidates.append((f"{euros}.99", base_score + bonus))
	
	# Try multiple preprocessing approaches (use contrast-enhanced gray, not original)
	inverted = 255 - contrast
	_, binary_otsu = cv2.threshold(inverted, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
	_, binary_manual_low = cv2.threshold(inverted, 100, 255, cv2.THRESH_BINARY)
	_, binary_manual_high = cv2.threshold(inverted, 140, 255, cv2.THRESH_BINARY)
	_, binary_inv = cv2.threshold(contrast, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
	
	# Apply morphological opening to remove noise
	kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
	binary_otsu_clean = cv2.morphologyEx(binary_otsu, cv2.MORPH_OPEN, kernel)
	
	binaries = [binary_otsu_clean, binary_otsu, binary_manual_low, binary_manual_high, binary_inv]
	
	# Try each binary with focused PSM modes
	# PSM 7 = single text line (best for prices)
	# PSM 13 = raw line without OSD
	for binary in binaries:
		for psm in [7, 8, 13, 6]:
			try:
				# Strategy 1: Try to get full price with decimal separator
				text_full = ocr_image(
					binary, 
					lang='en',
				...
				if matches:
					euros, cents = matches[0]
					price = f"{euros}.{cents}"
					try:
						price_val = float(price)
						if 0.10 <= price_val <= 99.99:
							# Score based on: common price endings, reasonable value
							score = 10
							if cents == '99':
								score += 8  # .99 is very common
							elif cents == '49':
								score += 4  # .49 is common
							elif cents in {'69', '59', '39'}:
								score += 2
							elif cents == '95':
								score += 2  # .95 is common
							elif cents in ['27', '29', '77', '79']:
								score -= 5  # Likely OCR errors for .99
							add_price_candidate(price, score)
					except ValueError:
						pass
				
				# Strategy 2: Digits only, then reconstruct
				text_numeric = ocr_image(
					binary, 
					lang='en',
					config=f"--oem 3 --psm {psm} -c tessedit_char_whitelist=0123456789"
				).strip()
				...
				# Reconstruct prices from digit sequences
				if 3 <= len(digits) <= 4:
					euros = digits[:-2] if len(digits) == 4 else digits[0]
					cents = digits[-2:]
					price = f"{euros}.{cents}"
					try:
						price_val = float(price)
						if 0.10 <= price_val <= 99.99:
							...
							if cents == '99':
								...
								score += 2
							# Penalize unusual endings
							elif cents in ['27', '29', '77',  '79']:
								score -= 3  # Likely OCR errors for .99
							add_price_candidate(price, score)
					except ValueError:
						pass
			except (RuntimeError, Exception):
				# Catch Tesseract timeout and other OCR errors - skip this combination
				continue
	
	# Return highest-scored price
	if price_candidates:
		# Count occurrences of each price
		price_counts = Counter([p for p, _ in price_candidates])
		
		# Re-score: add bonus for prices that appear multiple times
		rescored = []
		for price, score in price_candidates:
			count = price_counts[price]
			final_score = score + (count - 1) * 3  # +3 for each additional occurrence
			rescored.append((price, final_score))
