...
try:
    from src.ocr.pipeline.text_quality_analysis import TextQualityAnalyzer
...
def extract_product_text_with_nlp(img: np.ndarray, box: List[int]) -> Dict[str, any]:
	...
	def best_text_from_crop(crop_bgr: np.ndarray, region_name: str = "") -> Tuple[str, float, Dict]:
		...
		# Try different PSM modes - prioritize single line mode for titles
		psm_configs = [
	...
	for region_name, reg in regions:
		...
		for brand in ["FERREX", "TOPCRAFT", "WORKZONE", "UPAFASHION", "CRANE",
		              "BARISSIMO", "NESCAFE", "JACOBS", "ALDI", "LIDL"]:
		...
		if not brand_at_start:
			has_brand = any(brand in line.upper() for brand in [
				"FERREX", "TOPCRAFT", "WORKZONE", "UPAFASHION", "CRANE",
				"BARISSIMO", "NESCAFE", "JACOBS", "ALDI", "LIDL"
			])
			if has_brand:
				score += 100.0
	...
	if best_score > 0:
		...
		if NLP_AVAILABLE and ocr_text:
			try:
				nlp_result = extract_product_entities(ocr_text)
				...
				# Don't fall back to full OCR text - that defeats the purpose of NLP filtering
...
def postprocess_product_text(text: str) -> str:
	...
	# Final check - must have at least one real word (3+ letters)
	words = result.split()
	if not any(len(w) >= 3 and any(c.isalpha() for c in w) for w in words):
		return ""
